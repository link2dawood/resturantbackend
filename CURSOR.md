...
| 2026-04-02 | **Bank Statement Import (admin):** Routes under `role:admin,owner`: `GET/POST /bank-statement-imports`, batch show `GET /bank-statement-imports/batches/{importBatch}`, `POST .../bulk-update-coa`. `BankStatementImportController` + views in `resources/views/admin/bank-statement-imports/`. Bank fixed to **Bank of the West** via `BankStatementSupportedBank::resolveBankOfTheWestAccount($storeId)`. `BankImportController` parses CSV format `bank_west` (columns Account, ChkRef, Debit, Credit, Balance, Date, Description) and exposes `runBankStatementImportForAdmin`; `ImportBatch::bankTransactions()` relationship. Sample CSV: `docs/data/bank-of-the-west-activity-sample.csv`. |
