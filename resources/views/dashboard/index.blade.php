...
<div class="home-dashboard">
    ...
    <section class="home-hero">
        <div class="home-hero__content">
            ...
            <h1 class="home-hero__title">Restaurant analytics built for a fast first scan.</h1>
            <p class="home-hero__summary">
                Review sales movement, report coverage, fee exposure, and store performance from one lightweight workspace built on the existing dashboard.
            </p>

            <div class="home-hero__actions">
                @if($user->hasPermission('create_reports'))
                    <a href="{{ route('daily-reports.create') }}" class="home-action home-action--primary">
                        <i class="bi bi-plus-circle"></i>
                        Create Report
                    </a>
                @endif
                <a href="{{ route('daily-reports.index') }}" class="home-action">
                    <i class="bi bi-list-ul"></i>
                    View Reports
                </a>
                @if(($user->isAdmin() || $user->isOwner()) && $analytics['thirdParty']['statement_count'] > 0)
                    <a href="{{ route('admin.merchant-fees.third-party') }}" class="home-action">
            ...
            <div class="home-hero__meta">
                <span class="home-meta-pill">
                    ...
                    <strong>Updated</strong> {{ date('M j, Y g:i A') }}
                </span>
                <span class="home-meta-pill">
                    <i class="bi bi-person-badge"></i>
                    <strong>{{ $roleLabel }}</strong> workspace
                </span>
                @if($topStore)
                    <span class="home-meta-pill">
                        <i class="bi bi-shop"></i>
                        <strong>Top store</strong> {{ \Illuminate\Support\Str::limit($topStore->store_info, 26) }}
                    </span>
                @endif
            </div>
        </div>
        ...
        <div class="home-hero__rail">
            <div class="home-brief">
                <p class="home-brief__label">Today At A Glance</p>
                <div class="home-brief__list">
                    <div class="home-brief__row">
                        <div>
                            <p class="home-brief__title">Gross sales</p>
                            <p class="home-brief__meta">Captured from submitted daily reports</p>
                        </div>
                        <div class="home-brief__value">${{ number_format($analytics['overview']['today']['grossSales'], 0) }}</div>
                    </div>
                    <div class="home-brief__row">
                        <div>
                            <p class="home-brief__title">Reports received</p>
                            <p class="home-brief__meta">Operational coverage logged today</p>
                        </div>
                        <div class="home-brief__value">{{ $analytics['overview']['today']['reports'] }}</div>
                    </div>
                    <div class="home-brief__row">
                        <div>
                            <p class="home-brief__title">Net sales</p>
                            <p class="home-brief__meta">After discounts and adjustments</p>
                        </div>
                        <div class="home-brief__value">${{ number_format($analytics['overview']['today']['netSales'], 0) }}</div>
                    </div>
                </div>
            </div>

            <div class="home-signal">
                <p class="home-signal__label">Current Signal</p>
                @if($primaryInsight)
                    <h2 class="home-signal__headline">{{ $primaryInsight['title'] }}</h2>
                    <p class="home-signal__body">{{ $primaryInsight['message'] }}</p>
                    <div class="home-signal__footer">
                ...
                @elseif($bestDay)
                    ...
                    <div class="home-signal__footer">
                        ...
                        Based on the highest reported sales day
                    </div>
                @else
                    <h2 class="home-signal__headline">The dashboard is ready for the next reporting cycle.</h2>
                    <p class="home-signal__body">As new daily reports arrive, this home page will surface the clearest movement first.</p>
                    <div class="home-signal__footer">
    ...
    <section class="home-kpi-grid" aria-label="Key metrics">
        <article class="home-kpi">
            <div class="home-kpi__header">
                <span class="home-kpi__icon home-kpi__icon--green"><i class="bi bi-calendar-check"></i></span>
                <span class="home-kpi__trend {{ is_numeric($growthChange) ? ($growthChange >= 0 ? 'is-positive' : 'is-negative') : 'is-neutral' }}">
                    <i class="bi {{ is_numeric($growthChange) ? ($growthChange >= 0 ? 'bi-arrow-up-right' : 'bi-arrow-down-right') : 'bi-dash-lg' }}"></i>
                    {{ is_numeric($growthChange) ? (($growthChange >= 0 ? '+' : '') . $growthChange . '%') : 'No baseline' }}
                </span>
            </div>
            <p class="home-kpi__value">${{ number_format($analytics['overview']['today']['grossSales'], 0) }}</p>
            <p class="home-kpi__title">Today's Sales</p>
            <p class="home-kpi__meta">{{ $analytics['overview']['today']['reports'] }} reports submitted today</p>
        </article>

        <article class="home-kpi">
            <div class="home-kpi__header">
                <span class="home-kpi__icon home-kpi__icon--blue"><i class="bi bi-calendar-week"></i></span>
                <span class="home-kpi__trend is-neutral">
                    <i class="bi bi-bar-chart-line"></i>
                    Week to date
                </span>
            </div>
            <p class="home-kpi__value">${{ number_format($analytics['overview']['thisWeek']['grossSales'], 0) }}</p>
            <p class="home-kpi__title">This Week</p>
            <p class="home-kpi__meta">{{ $analytics['overview']['thisWeek']['reports'] }} reports captured</p>
        </article>

        <article class="home-kpi">
            <div class="home-kpi__header">
                <span class="home-kpi__icon home-kpi__icon--amber"><i class="bi bi-calendar-month"></i></span>
                <span class="home-kpi__trend {{ is_numeric($netSalesChange) ? ($netSalesChange >= 0 ? 'is-positive' : 'is-negative') : 'is-neutral' }}">
                    <i class="bi {{ is_numeric($netSalesChange) ? ($netSalesChange >= 0 ? 'bi-arrow-up-right' : 'bi-arrow-down-right') : 'bi-dash-lg' }}"></i>
                    {{ is_numeric($netSalesChange) ? (($netSalesChange >= 0 ? '+' : '') . $netSalesChange . '%') : 'Tracking' }}
                </span>
            </div>
            <p class="home-kpi__value">${{ number_format($analytics['overview']['thisMonth']['grossSales'], 0) }}</p>
            <p class="home-kpi__title">This Month</p>
            <p class="home-kpi__meta">{{ $analytics['overview']['thisMonth']['reports'] }} reports across locations</p>
        </article>

        <article class="home-kpi">
            <div class="home-kpi__header">
                <span class="home-kpi__icon home-kpi__icon--red"><i class="bi bi-graph-up-arrow"></i></span>
                <span class="home-kpi__trend {{ is_numeric($reportsChange) ? ($reportsChange >= 0 ? 'is-positive' : 'is-negative') : 'is-neutral' }}">
                    <i class="bi {{ is_numeric($reportsChange) ? ($reportsChange >= 0 ? 'bi-arrow-up-right' : 'bi-arrow-down-right') : 'bi-dash-lg' }}"></i>
                    {{ is_numeric($reportsChange) ? (($reportsChange >= 0 ? '+' : '') . $reportsChange . '%') : 'Pending' }}
                </span>
            </div>
            <p class="home-kpi__value">
                {{ is_numeric($growthChange) ? (($growthChange >= 0 ? '+' : '') . $growthChange . '%') : '--' }}
            </p>
            <p class="home-kpi__title">Monthly Growth</p>
            <p class="home-kpi__meta">Compared with the previous month</p>
        </article>
    </section>
    ...
    <section class="home-grid">
        ...
        <aside class="home-side">
            ...
            <article class="home-panel">
                <div class="home-panel__header">
                    <div>
                        <p class="home-section__eyebrow">Highlights</p>
                        <h2 class="home-panel__title">Top performing days</h2>
                        <p class="home-panel__subtitle">Highest reported sales days from the recent operating window.</p>
                    </div>
                </div>
                <div class="home-panel__body">
                            ...
                            @foreach($analytics['topDays']->take(4) as $index => $day)
                                <div class="home-list-item">
                                    ...
                                    <div class="home-list-item__value">
                                        ${{ number_format($day->gross_sales, 0) }}
                                        <div class="home-list-item__meta" style="margin-top:6px;">
                                            {{ $index === 0 ? 'Best day' : 'Rank #' . ($index + 1) }}
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    @else
                        <div class="home-empty">
                            ...
                            <p class="mb-0">Add more reports to rank strong days.</p>
                        </div>
                    @endif
                </div>
            </article>
            ...
            @if($user->isAdmin())
                @php
                    ...
                    $managers = \App\Models\User::where('role', \App\Enums\UserRole::MANAGER)->with('store')->orderBy('name')->take(3)->get();
                ...
                <article class="home-panel">
                    <div class="home-panel__header">
                        <div>
                            <p class="home-section__eyebrow">Admin</p>
                            <h2 class="home-panel__title">User impersonation</h2>
                            <p class="home-panel__subtitle">Search or jump into owner and manager accounts without leaving the dashboard.</p>
                        </div>
                    </div>
                    <div class="home-panel__body">
                        <button type="button" class="home-action home-action--primary w-100 mb-3" data-bs-toggle="modal" data-bs-target="#userSelectionModal">
                            <i class="bi bi-search"></i>
                            Search and login as user
                        </button>

                        <div class="home-admin-list">
                            @foreach($owners as $owner)
                                <div class="home-admin-card">
                                    <div class="home-admin-card__identity">
                                        <img src="{{ $owner->avatar_url ?? $defaultAvatar }}" alt="{{ $owner->name }}" class="home-admin-card__avatar">
                                        <div>
                                            <p class="home-admin-card__name">{{ $owner->name }}</p>
                                            <p class="home-admin-card__meta">{{ $owner->email }}</p>
                                        </div>
                                    </div>
                            ...
                            @foreach($managers as $manager)
                                <div class="home-admin-card">
                                    <div class="home-admin-card__identity">
                                        <img src="{{ $manager->avatar_url ?? $defaultAvatar }}" alt="{{ $manager->name }}" class="home-admin-card__avatar">
                                        <div>
                                            <p class="home-admin-card__name">{{ $manager->name }}</p>
                                            <p class="home-admin-card__meta">
                                                {{ $manager->email }}
                                                @if($manager->store)
                                                    <br>{{ \Illuminate\Support\Str::limit($manager->store->store_info, 28) }}
                                                @endif
                                            </p>
                                        </div>
                                    </div>
                                    <form method="POST" action="{{ route('impersonate.start', $manager) }}">
                                        @csrf
                                        <button type="submit" class="home-admin-card__button">
                                            <i class="bi bi-box-arrow-in-right"></i>
                                            Login
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-3" style="gap: 12px; flex-wrap: wrap;">
                            <span class="text-muted" style="font-size: 0.85rem;">{{ $totalOwners }} owners and {{ $totalManagers }} managers available</span>
                            <a href="{{ route('managers.index') }}" class="home-link-button">
            ...
            <article class="home-panel">
                <div class="home-panel__header">
                    <div>
                        <p class="home-section__eyebrow">Utility</p>
                        <h2 class="home-panel__title">Exports and actions</h2>
                        <p class="home-panel__subtitle">Common reporting tasks without leaving the page.</p>
                    </div>
                </div>
                <div class="home-panel__body">
                    <div class="home-actions-stack">
                        <div class="dropdown">
                            <button class="home-action home-action--primary w-100 dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                <i class="bi bi-download"></i>
                                Export Data
                            </button>
                            <ul class="dropdown-menu w-100">
                                <li>
                                    <a class="dropdown-item" href="{{ route('dashboard.export', ['format' => 'csv', 'type' => 'overview']) }}">
                                        <i class="bi bi-file-earmark-spreadsheet me-2"></i>Overview (CSV)
                                    </a>
                                </li>
                                <li>
                                    <a class="dropdown-item" href="{{ route('dashboard.export', ['format' => 'csv', 'type' => 'store_performance']) }}">
                                        <i class="bi bi-shop me-2"></i>Store Performance (CSV)
                                    </a>
                                </li>
                                <li>
                                    <a class="dropdown-item" href="{{ route('dashboard.export', ['format' => 'csv', 'type' => 'daily_trends']) }}">
                        ...
                        @if($user->hasPermission('create_reports'))
                            <a href="{{ route('daily-reports.create') }}" class="home-action">
                                <i class="bi bi-plus-circle"></i>
                                Create Report
                            </a>
                        @endif
                        ...
                        <button type="button" class="home-action" data-refresh-dashboard>
